...
export default function SalesChart({bookings,numDays}) {
  ...
  const data = allDates.map((date) => {
    return{label:format(date,"MMM dd"),totalSales:bookings.filter((booking) => isSameDay(date, new Date(booking.created_at))).reduce((acc,cur) => acc + cur.totalPrice,0),extrasSales:bookings.filter((booking) => isSameDay(date, new Date(booking.created_at))).reduce((acc,cur) => acc + cur.extrasPrice,0)}
  });
const colors = isDarkMode
  ...
  return (
   <StyledSalesChart>
    <Heading as="h2">sales from {format(allDates.at(0),"MMM dd")} to {format(allDates.at(-1),"MMM dd")}</Heading>
    ...
    <AreaChart
      ...
      data={fakeData}
    >
      <XAxis dataKey="label" tick={{ stroke: colors.text}} tickLine={{stroke: colors.text}}/>
      <YAxis unit="₹"tick={{ stroke: colors.text}} tickLine={{stroke: colors.text}}/>
      <Tooltip contentStyle={{backgroundColor: colors.background}}/>
      <Legend />
      <CartesianGrid strokeDasharray="4"/>
      <Area type="monotone" dataKey="totalSales" stroke={colors.totalSales.stroke} fill={colors.totalSales.fill} strokeWidth={2} name="Total Sales" unit="₹" />
      <Area type="extraSales" dataKey="extrasSales"stroke={colors.totalSales.stroke} fill={colors.totalSales.fill} strokeWidth={2} name="Total Sales" unit="₹" />
      </AreaChart>
    </ResponsiveContainer>
   </StyledSalesChart>
  )
